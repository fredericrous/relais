...
For the per-session cap to count agents rather than spawns, the seat has to come back when the call ends, and `handle` gives it back: a `PostToolUse` or `PostToolUseFailure` on the Agent tool carries the same `tool_use_id` its spawn did, so the dispatch id derives identically and the end finds exactly what the start took — no pairing, nothing remembered between firings. It is settled with `None` rather than a figure, because a hook payload carries no usage: `None` books the reservation as a lower bound and marks the run uncertain, which is what not knowing looks like when it is recorded honestly, where zero would be a claim that the agent was free. Without this the seat would lapse only with the unbound dispatch's `LEASE_GRACE` — five minutes — and `max_active_agents_per_session` would cap spawns per rolling five minutes while the refusal told the session to "retry when a running agent finishes", advice that would free nothing.
